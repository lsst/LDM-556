...
\paragraph{DataRepository Layering}\hfill  % Force subsequent text onto new line
...
\paragraph{DataRepository Layering: Data Release and Science Platform}\hfill  % Force subsequent text onto new line
...
\paragraph{DataRepository Layering: Data Release and external hardware}\hfill  % Force subsequent text onto new line
...
\paragraph{DataRepository Layering: Data Release Production}\hfill  % Force subsequent text onto new line
...
\paragraph{DataRepository Layering: Data Release Production intermediates to external hardware}\hfill  % Force subsequent text onto new line
...
\paragraph{DataRepository Layering: Science Platform}\hfill  % Force subsequent text onto new line
...
\paragraph{DataRepository Layering: Science Platform to external hardware}\hfill  % Force subsequent text onto new line
...
\textbf{Specification:}
The Pipeline API design shall permit a supervisory framework to execute any Pipeline, based solely on information obtained programmatically from the Pipeline specification, against a data specification provided by a user (see in particular \hyperref[DMS-MWST-REQ-0021]{DMS-MWST-REQ-0021} and \hyperref[DMS-MWST-REQ-0027]{DMS-MWST-REQ-0027}).
...
\textbf{Specification:}
(Proposed to be deleted) The API for the execution of an individual processing step on specific data shall allow the caller to supply a Butler instance for the step's use.
...
\textbf{Specification:}
The Pipeline design shall support programmatic insertions (before the "Pre-flight" phase) of additional processing steps to an already-specified Pipeline’s processing sequence. The provenance mechanism (see \hyperref[DMS-MWST-REQ-0024]{DMS-MWST-REQ-0024}) must be capable of capturing these additional steps.
